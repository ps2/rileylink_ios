...
+ (uint8_t) computeCRC8:(NSData*)data {
    uint8_t crc = 0;
...
- (instancetype)initWithRFSPYResponse:(NSData*)data
{
    self = [super init];
    if (self) {
        if (data.length > 0) {
...
- (NSData*)encodedData {
    ...
    // TODO; we should be able to avoid this copy by accessing the CRC
    // in the loop below when i == data.length
    NSMutableData *dataPlusCrc = [_data mutableCopy];
    unsigned char crc = [RFPacket computeCRC8:_data];
