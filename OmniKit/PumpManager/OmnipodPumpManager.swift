            ...
            self.podComms.runSession(withName: "Enact Temp Basal", using: rileyLinkSelector) { (result) in
                ...
                do {
                    ...
                    if duration < .ulpOfOne {
                        ...
                        let result = session.setTempBasal(rate: rate, duration: duration, acknowledgementReminder: false, confidenceReminder: false, programReminderInterval: 0)
                        let basalStart = Date()
            ...
            self.podComms.runSession(withName: "Save Basal Profile", using: self.rileyLinkDeviceProvider.firstConnectedDevice) { (result) in
                do {
                    ...
                    case .success(let session):
                        ...
                        let _ = try session.setBasalSchedule(schedule: schedule, scheduleOffset: scheduleOffset, acknowledgementReminder: false, confidenceReminder: false, programReminderInterval: 0)
                        self.state.basalSchedule = schedule
                        completion(nil)
                    case .failure(let error):
                        throw error
