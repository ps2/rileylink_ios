...
class PodStateTests: XCTestCase {
    ...
    func testNonceValues() {
        var podState = PodState(address: 0x1f000000, activatedAt: now, expiresAt: now.addingTimeInterval(podSoftExpirationTime), timeZone: .currentFixed, piVersion: "1.1.0", pmVersion: "1.1.0", lot: 42560, tid: 661771)
    ...
    func testNonceSync() {
        do {
            ...
            var podState = PodState(address: 0x1f00ee86, activatedAt: now, expiresAt: now.addingTimeInterval(podSoftExpirationTime), timeZone: .currentFixed, piVersion: "1.1.0", pmVersion: "1.1.0", lot: config.lot, tid: config.tid)
    ...
    func testNonceSync2() {
        do {
            ...
            var podState = PodState(address: 0x1f0b3557, activatedAt: now, expiresAt: now.addingTimeInterval(podSoftExpirationTime), timeZone: .currentFixed, piVersion: "1.1.0", pmVersion: "1.1.0", lot: config.lot, tid: config.tid)
